...
const Register = () => {
  ...
  return <div className="min-h-screen gradient-secondary flex items-center justify-center p-3 md:p-4 py-8 md:py-12 relative overflow-hidden">
      {/* Animated Background */}
      ...
      {[...Array(15)].map((_, i) => <div key={i} className="absolute bg-white/20 rounded-full animate-pulse-slow hidden sm:block" style={{
      ...
      <Card className="w-full max-w-lg relative z-10 shadow-2xl border-2 border-gold/20 backdrop-blur-xl bg-white/95 animate-scale-in mx-4">
        <CardHeader className="space-y-3 md:space-y-4 text-center pb-6 md:pb-8">
          ...
          <div className="flex flex-wrap justify-center gap-2 md:gap-3 text-xs px-4">
            ...
            icon: CheckCircle2,
            text: 'No Credit Card'
          }].map((item, index) => <div key={index} className="flex items-center gap-1.5 text-gold font-medium">
        ...
        <CardContent className="px-4 md:px-6">
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-3 md:space-y-4">
              ...
              field
                    ...
                    <FormControl>
                      <div className="relative">
                        ...
                        <Input placeholder="John Doe" className="pl-10 md:pl-11 h-11 md:h-12 border-2 focus:border-gold transition-colors text-sm md:text-base" {...field} />
                      </div>
                    </FormControl>
                    <FormMessage />
                  </FormItem>} />
              ...
              <Button type="submit" className="w-full gradient-gold text-navy font-bold h-11 md:h-12 text-sm md:text-base rounded-xl hover:shadow-gold transition-all duration-300 relative overflow-hidden group mt-4 md:mt-6 border-2 border-gold/20" disabled={loading}>
                {loading ? <>
